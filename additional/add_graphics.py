...
import numpy as np
import additional.add_regr as regrs


def plot_graph_conf_int_2(y, x, regr_name='lin', x_label='', y_label='', file_path=''):
    n = 10
    regr = regrs.regression_factory(regr_name, y, x)
    eq = regr.equation
    func = regr.func_ap
    x_ = np.linspace(min(x), max(x), n)
    y_ap = [func(i) for i in x_]

    conf_int = regr.conf_int

    mean_up = conf_int.get_mean_app_up_list()
    mean_low = conf_int.get_mean_app_low_list()
    ind_up = conf_int.get_ind_app_up_list()
    ind_low = conf_int.get_ind_app_low_list()

    fig, ax = plt.subplots()

    ax.plot(x, y, 'o')
    ax.plot(x_, y_ap, '-', label=eq)
    ax.fill_between(mean_up[0], mean_up[1], mean_low[1], alpha=0.5)
    ax.fill_between(ind_up[0], ind_up[1], ind_low[1], alpha=0.2)
    ...
    try:
        ...
        print('Проблема с файлом file path: {}'.format(file_path))
    plt.show()
...
def plot_simple_graph(list_x, list_y, label_x='', label_y=''):
    plt.fig = plt.figure(figsize=[10.0, 7.0])
    plt.axes(xlabel=label_x, ylabel=label_y)
